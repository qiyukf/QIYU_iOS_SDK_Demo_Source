...
#import "YSFDatabasePool.h"
    ...
    #define YSFDBAutorelease(__v) ([__v autorelease]);
    #define YSFDBReturnAutoreleased YSFDBAutorelease

    #define YSFDBRetain(__v) ([__v retain]);
    #define YSFDBReturnRetained YSFDBRetain

    #define YSFDBRelease(__v) ([__v release]);

    #define YSFDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define YSFDBAutorelease(__v)
    #define YSFDBReturnAutoreleased(__v) (__v)

    #define YSFDBRetain(__v)
    #define YSFDBReturnRetained(__v) (__v)

    #define YSFDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define YSFDBDispatchQueueRelease(__v)
    #else
        #define YSFDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^YSFDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `<YSFStatement>` - A wrapper for `sqlite_stmt`.
...
@interface YSFDatabase : NSObject  {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(YSFDBExecuteStatementsCallbackBlock)block;
...
- (YSFResultSet *)executeQuery:(NSString*)sql, ...;
...
- (YSFResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (YSFResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (YSFResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (YSFResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `YSFStatement`, but rather with `<FMDatabase>` and `<FMResultSet>` only.
...
@interface YSFStatement : NSObject {
