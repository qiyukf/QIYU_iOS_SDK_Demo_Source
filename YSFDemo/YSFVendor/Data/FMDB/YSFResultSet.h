...
@interface YSFResultSet : NSObject {
    YSFDatabase          *_parentDB;
    YSFStatement         *_statement;
...
/** `YSFStatement` used by result set. */

@property (atomic, retain) YSFStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<YSFStatement>`
 
 @param statement A `<YSFStatement>` to be performed
...
+ (instancetype)resultSetWithStatement:(YSFStatement *)statement usingParentDatabase:(YSFDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(YSFDatabase *)newDb;
