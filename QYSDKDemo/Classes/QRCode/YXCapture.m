...
static const void * const YSF_YXCaptureOutputQueueSpecificKey = &YSF_YXCaptureOutputQueueSpecificKey;
...
- (void)dealloc {
    //判断当前queue，防止同步死锁
    if (dispatch_get_specific(YSF_YXCaptureOutputQueueSpecificKey)) {
        [self removeDelegateForCaptureSession];
    } else {
...
- (void)removeDelegateForCaptureSession {
...
- (id)init
{
    ...
    if (self) {
        ...
        dispatch_queue_set_specific(_captureOutputBufferQueue, YSF_YXCaptureOutputQueueSpecificKey, (void *)YSF_YXCaptureOutputQueueSpecificKey, NULL);
    }
