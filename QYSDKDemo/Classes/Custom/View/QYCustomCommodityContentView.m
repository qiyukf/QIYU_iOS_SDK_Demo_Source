...
@interface QYCustomCommodityContentView ()
...
@implementation QYCustomCommodityContentView
...
- (void)refreshData:(QYCustomModel *)model {
    ...
    if (![model isKindOfClass:[QYCustomCommodityModel class]]) {
        return;
    }
    
    QYCustomCommodityModel *ticModel = (QYCustomCommodityModel *)model;
    QYCustomCommodityInfo *ticMessage = (QYCustomCommodityInfo *)ticModel.message;
    NSDictionary *dict = [ticMessage.jsonData qy_toDict];
    
    self.titleLabel.text = [dict qy_jsonString:@"title"];
    self.priceLabel.text = [dict qy_jsonString:@"price"];
    self.descLabel.text = [dict qy_jsonString:@"desc"];
    [self.button setTitle:[dict qy_jsonString:@"buttonTitle"] forState:UIControlStateNormal];
    ...
    NSArray *tagArray = [dict qy_jsonStringArray:@"tag"];
    if ([tagArray count]) {
